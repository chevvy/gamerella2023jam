...
func get_ready():
	...
	twe.tween_property(get_ready_title, "position", Vector2(800, 0), 1).set_trans(
		Tween.TRANS_SPRING
	)

	_bar_anim()
	twe.tween_interval(1)
	twe_text.tween_property(energy_title, "position", Vector2(45, 72), 0.2).set_trans(
		Tween.TRANS_SPRING
	)
	twe_text.tween_property(depth_cont, "position", Vector2(0, 0), 0.2).set_trans(
		Tween.TRANS_SPRING
	)
...
func _bar_anim():
	...
	twe_ui.tween_property(progress_bar, "position", Vector2(45, 20), 0.5).set_trans(
		Tween.TRANS_SPRING
	)
